  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-(--color-light-card-border) bg-bg-shell px-3 py-3 dark:border-(--color-dark-card-border) dark:bg-dark-shell">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold text-primary dark:text-dark-primary">{title}</p>
        <p className="truncate text-xs text-muted dark:text-dark-muted">{subtitle}</p>
      </div>
      {badge ? (
          ...
          >
            ...
            >
              <Field
                ...
                placeholder={t("adminProjects.form.project.placeholders.projectName")}
                register={register("projectName", {
                  required: t("adminProjects.form.project.validation.projectName"),
                })}
            ...
            >
                  ...
                  render={({ field, fieldState }) => (
                    <div className="flex flex-col gap-2">
                      <SearchableSelect
                        ...
                        placeholder={
                          groupsLoading
                            ? t("adminProjects.form.project.placeholders.loadingGroups")
                            : t("adminProjects.form.project.placeholders.group")
                        }
                        searchPlaceholder={t("adminProjects.form.project.placeholders.groupSearch")}
                      ...
                      {selectedGroup ? (
                        <PersonTile
                          ...
                          subtitle={t("adminProjects.form.project.selector.groupRow")}
                          badge={t("adminProjects.form.project.stats.group")}
                        />
                      ) : null}
                      {fieldState.error?.message ? (
                        <p className="text-[11px] text-error dark:text-red-400">{fieldState.error.message}</p>
                      ) : null}
                    </div>
                  )}
                />
              ...
              <div className="rounded-xl border border-(--color-light-card-border) bg-bg-shell p-4 dark:border-(--color-dark-card-border) dark:bg-dark-shell">
                ...
                <Controller
                  ...
                  control={control}
                  rules={{
                    required: t("adminProjects.form.project.validation.teacher"),
                  }}
                  render={({ field, fieldState }) => (
                    <div className="flex flex-col gap-2">
                      <SearchableSelect
                      ...
                      {selectedTeacher ? (
                        <PersonTile
                          ...
                          badge={t("adminProjects.form.project.selector.ownerBadge")}
                        />
                      ) : null}
                      {fieldState.error?.message ? (
                        <p className="text-[11px] text-error dark:text-red-400">{fieldState.error.message}</p>
  ...
  if (isEdit && projectLoading) {
    return (
      <div className="flex min-h-screen flex-1 items-center justify-center bg-light-app-bg p-4 dark:bg-dark-card-bg">
        <p className="text-sm text-muted dark:text-dark-muted">{t("adminProjects.form.project.loading")}</p>
      </div>
    );
  }
  ...
  if (isEdit && !projectLoading && !project) {
    return (
      <div className="flex min-h-screen flex-1 flex-col items-start gap-4 bg-light-app-bg p-4 dark:bg-dark-card-bg md:p-5">
        <Button variant="tertiary" onClick={() => navigate("/admin/projects?tab=projects")}>
        ...
        <div className="rounded-3xl border border-(--color-light-card-border) bg-(--color-light-card-bg) p-6 shadow-sm dark:border-(--color-dark-card-border) dark:bg-(--color-dark-card-bg)">
          <p className="text-sm text-secondary dark:text-dark-secondary">{t("adminProjects.form.project.notFound")}</p>
        </div>
      </div>
    );
  }
          ...
          <header className="mb-8 flex flex-col gap-4 border-b border-default pb-6 dark:border-dark-default sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-bold tracking-tight text-primary sm:text-[1.65rem] dark:text-dark-primary">
                {isEdit ? t("adminProjects.form.project.editTitle") : t("adminProjects.form.project.createTitle")}
              </h1>
                      ...
                      {i < steps.length - 1 ? (
                        <span
                          className={[
                            ...
                            done ? "bg-chart-success/50 dark:bg-chart-success/35" : "bg-light-border-subtle dark:bg-(--color-dark-border-subtle)",
                          ].join(" ")}
                      ...
                      <div className="min-w-0 pt-0.5">
                        <span
                          className={[
                            ...
                            active ? "text-light-timeline-text dark:text-(--color-dark-timeline-text)" : "",
                            done ? "text-chart-success dark:text-chart-success" : "",
                          ].join(" ")}
                        >
                          {t(s.titleKey)}
                        </span>
                ...
                <div className="overflow-hidden rounded-2xl border border-(--color-light-success-border) bg-light-success-bg p-4 sm:p-5 dark:border-dark-success-border dark:bg-dark-success-bg">
                  <div className="mb-3">
                    <p className="text-sm font-semibold text-light-success-text dark:text-(--color-dark-success-text)">
                      {t("adminProjects.form.project.progress.label", { pct: progressPct })}
                    </p>
