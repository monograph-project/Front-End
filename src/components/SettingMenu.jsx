...
/**
 * SettingsMenu — Reusable dark sidebar menu component.
 *
 * Props:
 *   items: Array<{ label: string, icon: ReactNode, onClick?: (label, index) => void }>
 *   onItemClick?: (label: string, index: number) => void   — global fallback listener
 *   defaultActive?: number   — index of the initially active item (default: null)
...
export function SettingsMenu({
  ...
  const handleClick = (item, index) => {
    setActive(index);
    item.onClick?.(item.label, index);
    onItemClick?.(item.label, index);
  };

  return (
    <div
      className={`flex flex-col w-[200px] rounded-md overflow-hidden ${className}`}
    ...
    >
      {items.map((item, index) => (
        <button
          ...
          className={`
            relative flex items-center gap-2 px-3 py-[10px] text-sm text-white
            border-none cursor-pointer rounded-[4px] transition-colors duration-150
            ${active === index ? "bg-[#1A1F24]" : "bg-transparent hover:bg-[#21262C]"}
          `}
        >
          <span
            className="absolute left-[-10px] top-[10%] w-[5px] rounded-md bg-[#2F81F7] transition-opacity duration-150"
            style={{ height: "80%", opacity: active === index ? 1 : 0 }}
          />
          {item.icon}
          {item.label}
        </button>
...
const DEFAULT_ITEMS = [
  {
    ...
    onClick: (label) => console.log("Per-item click:", label),
  },
